...
async function fetchCompetitionSnapshot(sourceUrl) {
  ...
  const urls = [proxyUrl, allOriginsUrl, sourceUrl];
  let lastError = null;
  ...
  for (const url of urls) {
    try {
      ...
      if (text && (text.includes("Current standings") || text.includes("| Pos | Club |"))) {
        return text;
      }
...
async function fetchCompetitionFromFeed() {
  ...
  if (data.standings.length < 6 || data.nextGames.length < 1) {
